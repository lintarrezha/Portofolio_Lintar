...
export default function MobileHeader() {
  ...
  return (
    <div className="md:hidden flex fixed top-[0.25rem] right-2 h-12 py-2 z-[999]">
      <button
      ...
      <motion.div className={`${open ? 'block' : 'hidden'} bg-white border-none overflow-hidden top-2 mr-5 ml-2 w-56 right-12 absolute rounded-lg`} animate={open ? 'open' : 'closed'} variants={variants}>
        {links.map((link) => (
          <motion.a
            key={link.name}
            href={link.hash}
